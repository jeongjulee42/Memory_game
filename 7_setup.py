    ...
    for button in number_buttons:
        if button.collidepoint(pos):
            if button == number_buttons[0]:
                ...
                game_over()  # 현재 레벨 보여주고 게임오버
            break

    if len(number_buttons) == 0:  # 모든 숫자를 다 맞혔다면 레벨을 높여 다시 시작 화면으로 감
        start = False
        hidden = False
        curr_level += 1
        setup(curr_level)  # 셋업함수 호출 -> 다시 모든게 설정된다.
...
def game_over():
    ...
    screen.fill(BLACK)  # 보여지고있는 숫자들을 모두 지움. 겹치면 안예쁘다.
    screen.blit(msg, msg_rect)
